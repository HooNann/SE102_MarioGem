...
void CPlayScene::_ParseSection_ANIMATIONS(string line) {
  ...
  if (tokens.size() < 3)
    return; // skip invalid lines - an animation must at least has 1 frame and 1
            // frame time
  ...
  for (int i = 1; i < tokens.size();
       i += 2) // why i+=2 ?  sprite_id | frame_time
  {
...
void CPlayScene::Update(DWORD dt) {
  // We know that Mario is the first object in the list hence we won't add him
  // into the colliable object list TO-DO: This is a "dirty" way, need a more
  // organized way
